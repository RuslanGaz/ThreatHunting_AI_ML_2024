...
``` r
sql <- "SELECT min(bytes),max(bytes),max(bytes) - min(bytes) as anomaly, avg(bytes), port,count(port) FROM read_parquet([?]) where (src LIKE '12.%' or src LIKE '13.%' or src LIKE '14.%') and (dst LIKE '12.%' or dst LIKE '13.%' or dst LIKE '14.%') group by port order by anomaly asc limit 1"
df <- dbGetQuery(con, sql, list(PARQUET_FILE1))
df %>% select(port)
```
      port
